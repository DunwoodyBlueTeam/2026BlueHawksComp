...
Write-Host "=== Service and Process Audit ===" -ForegroundColor Cyan
...
Write-Host "`nAutomatic Services that are Stopped (potential issues):" -ForegroundColor Red
...
Write-Host "`nProcesses listening on network:" -ForegroundColor Yellow
...
Get-ScheduledTask | Export-Csv -Path "ScheduledTasks_$timestamp.csv" -NoTypeInformation
Write-Host "`nData exported to Services_$timestamp.csv and ScheduledTasks_$timestamp.csv" -ForegroundColor Green
