...
Write-Host "=== CCDC Security Baseline Hardening ===" -ForegroundColor Cyan
Write-Host "This script will apply basic security hardening" -ForegroundColor Yellow
$confirm = Read-Host "Continue? (y/n)"
if ($confirm -ne 'y') { exit }

# Enable Windows Firewall on all profiles
Write-Host "`n[+] Enabling Windows Firewall..." -ForegroundColor Green
Set-NetFirewallProfile -Profile Domain,Public,Private -Enabled True

# Disable SMBv1
Write-Host "[+] Disabling SMBv1..." -ForegroundColor Green
...
Write-Host "[+] Configuring password policy..." -ForegroundColor Green
...
foreach ($svc in $servicesToDisable) {
    try {
        ...
        Write-Host "  - Could not disable $svc" -ForegroundColor Yellow
    }
}
...
Write-Host "Review changes and test critical services!" -ForegroundColor Yellow